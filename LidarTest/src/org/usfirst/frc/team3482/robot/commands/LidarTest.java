...
public class LidarTest extends SensorBase implements PIDSource {
    I2C i2cDevice;
    Thread t;
    ...
    public static class Settings {
        ...
        double signalRateLimit;
        int sigmaEstimateLimate;
        int timingBudgetInMS;
        int preRangeVcselPeriod;
        int finalRangeVcselPeriod;
        String fwVersion;
        String stPalApi;
        offsetCalFlag offsetCalibration;
        ledIndicator ledIndicatorMode;
        boolean watchdogTimer;
        int offsetCalibrationValue;
        int crosstalkCalibrationValue;
        ...
        Settings(byte[] response) {
            ...
            signalRateLimit = ByteBuffer.wrap(response, 2, 2).getInt() / 65536.0;
            /* Process the 4th byte */
            sigmaEstimateLimate = response[4];
            /* Process the 5th & 6th bytes */
            timingBudgetInMS = ByteBuffer.wrap(response, 5, 2).getInt();
     ...
     */
    public LidarTest(Port port, int kAddress) {
    ...
    public Settings querySettings() {
        byte[] dataBuffer = new byte[22];

        i2cDevice.write(0x51, 0x1);
        i2cDevice.readOnly(dataBuffer, 22);
